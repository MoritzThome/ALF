...
%   lll}
   \toprule
    ...
    \texttt{Calc\_Fl}          & \path{logical}          & Explicitly calculate flavors (optional) \\
     \bottomrule
   \toprule
...
\begin{center}
 \begin{tabular}{ p{0.25\columnwidth} p{0.51\columnwidth} p{0.15\columnwidth} } 
   ...
   \texttt{Pr\_obs}   &
      ...
      Reconstruction of the weight for `inactive' flavors. &  \ref{sec:flavor_sym}  \\    
      \texttt{GR\_reconstruction}   &
      Reconstruction of the Greens function GR for the `inactive' flavors. &  \ref{sec:flavor_sym}  \\    
      \texttt{GRT\_reconstruction}   &
      Reconstruction of the time-displaced Greens functions G0T and GT0. & \ref{sec:flavor_sym}
 \\\bottomrule   
 \end{tabular}
...
Here, a pair of flavors $(n_\mathrm{fl},\bar{n}_\mathrm{fl})$ is related by a unitary or anti-unitary symmetry, including particle-hole transformations, such that
\begin{equation}
...
This symmetry allows to reconstruct one flavor, say $\bar{n}_\mathrm{fl}$, from the other, $n_\mathrm{fl}$. For unitary symmetries, the weight is given by $Z(\bar{n}_\mathrm{fl})=Z(n_\mathrm{fl})$, while anti-unitary symmetries lead to $Z(\bar{n}_\mathrm{fl})=Z(n_\mathrm{fl})^*$. This relation has to be provided by the user in  \texttt{weight\_reconstruction}. Upto entry, all weights $Z(n_\mathrm{fl})$ have be explicilty calculated by ALF and the user has to fill all `inactive' flavors $\bar{n}_\mathrm{fl}$ of the array $Z$.
Similarly, the subroutines \texttt{GR\_reconstruction} and \texttt{GRT\_reconstruction} have to be override to provide the symmetry reconstruction of the `inactive' flavors $\bar{n}_\mathrm{fl}$ of the equal-time and time-displaced Greens-function out of $n_\mathrm{fl}$, respectively.
