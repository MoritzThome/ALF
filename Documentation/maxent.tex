...
In many   case,  one  would like  to  compute  the   integration 
\begin{equation}
(F,A) = \int d \omega F(\omega) A(\omega)
\end{equation} 
for  a  user  defined  function $F(\omega) $   and  the  spectral  function  produced  in the   stochastic  MaxEnt.  An  example  
where  this  can  become handy  is  the  calculation of  the    the    quantum  Fisher  information (QFI) \cite{Hauke16}  or susceptibilities. 
As   seen   above,  the   stochastic MaxEnt,    encodes  the  image $n(x)$ as  a  set  of   Dirac $\delta$-functions  \ref{eqn:n_parametrization} thus  
rendering the  integration   very  convenient to  carry  out.  Since we  stochastically  sample  the image, we  can equally  estimate  
...
One  can use  the above  to compute  very  efficiently susceptibilities   that  are   typically  of the form: 
\begin{equation}
   \chi  =   \int_{0}^{\beta } d \tau   g(\tau)      =   \int d  \omega \underbrace{ \int_{0}^{\beta}  d  \tau  K(\tau, \omega)  }_{\equiv F(\omega)}   \, A (\omega). 
\end{equation}
Here,  the   analytical  continuation is  used  as   an  optimal fit  function  to  the  imaginary time  data,  allowing to  minimize   finite  
